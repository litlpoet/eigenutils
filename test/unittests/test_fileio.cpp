...
//   if (os_mat.is_open()) {
//     os_mat.write(reinterpret_cast<char *>(mat_write.data()),
//     sizeof(mat_write)); os_mat.close();
//   } else {
...
//     ss_out.write(reinterpret_cast<char *>(mat_write.data()),
//     sizeof(mat_write)); ss_out.seekg(0); e.encode(ss_out, os_mat);
//     os_mat.close();
//   } else {
//     std::cout << "Unable to open file!" << std::endl;
//   }
...
//   for (auto &mat : mat_list_write)
//     mat = Eigen::Matrix4d::Random();

//   EXPECT_EQ(sizeof(double) * 4 * 4 * 10, sizeof(Transf) * 10);

//   std::ostream os_mat("mat_list_write.bin", std::ios::out |
//   std::ios::binary); if (os_mat.is_open()) {
...
//   std::ifstream is_mat("mat_list_write.bin", std::ios : in |
//   std::ios::binary); if (is_mat.is_open()) {
